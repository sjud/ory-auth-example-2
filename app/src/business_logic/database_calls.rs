...
#[tracing::instrument(err)]
pub async fn create_post(pool:&SqlitePool,user_id:String,content:String) -> Result<PostRow,Error> {
    sqlx::query_as::<_,PostRow>("INSERT INTO posts (post_id,user_id,content) VALUES (?,?,?) RETURNING post")
        .bind(uuid::Uuid::new_v4().to_string())
        .bind(user_id)
        .bind(content)
        .fetch_one(pool)
...
#[tracing::instrument(err)]
pub async fn read_user_by_identity_id(pool:&SqlitePool,identity_id:String) -> Result<UserRow,Error> {
...
impl UserRow {
    #[tracing::instrument(err)]
    pub async fn from_identity(pool:&SqlitePool,identity:&Identity) -> Result<Self,sqlx::Error> {
